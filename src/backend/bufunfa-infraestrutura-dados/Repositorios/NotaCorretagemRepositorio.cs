...
{
    public class NotaCorretagemRepositorio : INotaCorretagemRepositorio
    {
        private readonly EfDataContext _efContext;

        public NotaCorretagemRepositorio(EfDataContext efContext)
        {
            _efContext = efContext;
        }

        public async Task<NotaCorretagem> ObterPorId(int idNota)
        {
            return await _efContext.NotaCorretagem
                .Include(x => x.Conta)
                .Include(x => x.Lancamentos)
                   .ThenInclude(x => x.Conta)
                .Include(x => x.Lancamentos)
                   .ThenInclude(x => x.Categoria.CategoriaPai)
                .FirstOrDefaultAsync(x => x.Id == idNota);
        }
        ...
        public async Task Inserir(NotaCorretagem nota)
        {
            await _efContext.AddAsync(nota);
        }

        public void Atualizar(NotaCorretagem nota)
        {
            _efContext.Entry(nota).State = EntityState.Modified;
        }

        public void Deletar(NotaCorretagem nota)
        {
            _efContext.NotaCorretagem.Remove(nota);
        }
